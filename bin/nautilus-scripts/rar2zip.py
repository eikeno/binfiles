...
logger = logging.getLogger('rar2zip')
hdlr = logging.FileHandler('/tmp/rar2zip.log')
...
def rar2zip(file_path):
	_tempdir = tempfile.mkdtemp(prefix='rar2zip_py')
	os.chdir(_tempdir)
	ret, out, err = cmd_run("unrar x -idp '%s'" % file_path)

	file_base = os.path.splitext(os.path.basename(file_path))[0]

	zcmd = "zip -r '" + OP_DIR + "/" + file_base + "'.zip *"
...
for file_path in SELECTED_FILE_PATHS.split('\n'):
	...
	RET, OUT, ERR, TEMPD = rar2zip(file_path)
